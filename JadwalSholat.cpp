...
void sholatCal(int Year,int Month,int Days, float TimeZone, float Latitude, float Longitude, float Altitude)
  {
//    float EJD = E_Julian_date(2020,7,30,106.7791);
    float EJD = E_Julian_date(Year,Month,Days,Longitude);
    float Decl=Dql(EJD);
    float EqOfTime=EqT(EJD);
    Pray_Time(TimeZone, Latitude, Longitude, Altitude,Decl, EqOfTime );
//    Pray_Time(7, -6.2311, 106.7791,45,Decl, EqOfTime );
  }
...
//    sholatT[4] = BaseTime + (float)Prm.IH/60 + (float)Prm.IL/60; // I = Ihti / menambahkan menit/ koreksi waktu sholat
    sholatT[4] = BaseTime;
    // Ashr
...
//  {
//    hijir_date BuffDate;
////    long nday = Days(Y, M, D)+ Prm.CH + cor; //sebelum dibuat library
